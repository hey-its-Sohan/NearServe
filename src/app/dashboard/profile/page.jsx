...
export default function ProfilePage() {
  ...
  const name = sUser.name || "User";
  const email = sUser.email || "you@example.com";
  const role = (sUser.role || "customer").toLowerCase(); 
  const avatarSrc =
    preview ||
    sUser.image ||
  ...
  const roleStyle =
    {
      customer: "bg-secondary/10 text-secondary border border-secondary/30",
      provider: "bg-primary/10 text-primary border border-primary/30",
      admin: "bg-accent/10 text-accent border border-accent/30",
    }[role] || "bg-muted text-foreground/80 border border-border";
      ...
      <section className="space-y-6">
        <header className="flex items-center justify-between flex-wrap gap-3">
          <div>
            ...
            <br />
            <p className="text-gray">Loading your information…</p>
          </div>
          <div className="px-3 py-1.5 rounded-full text-sm font-medium bg-muted text-foreground/80 border border-border">
            Loading…
          </div>
        </header>

        <div className="grid gap-6 lg:grid-cols-[320px,1fr]">
          <div className="card-animate h-60 animate-pulse" />
          <div className="card-animate h-60 animate-pulse" />
        </div>
      </section>
    );
  ...
  return (
    <section className="space-y-6">
      ...
      <header className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className={`px-3 py-1.5 rounded-full text-sm font-medium capitalize ${roleStyle}`}>
          {roleLabel}
        </div>
      </header>
        ...
        <div className="card-animate">
          <div className="grid gap-5 sm:grid-cols-2">
            ...
            <div className="sm:col-span-2">
              ...
              <div className={`mt-2 inline-flex items-center gap-2 px-3 py-1.5 rounded-full capitalize ${roleStyle}`}>
